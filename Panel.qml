...
Panel {
    id: root
    moduleName: 'nixfred.disk-pulse'
    ipcTarget: 'nixfred.disk-pulse'
    manageIpc: false
    ...
    readonly property string helper: String(Qt.resolvedUrl('disk_pulse.py')).replace(/^file:\/\//,'')
    ...
    function setMode(value) {
        root.settings=Object.assign({}, root.settings, {displayMode:Model.clamp(value,0,5)})
        if(root.bar && root.bar.shell) root.bar.shell.updateEntryInline(root.moduleName,root.settings)
    }
    function setMountpoint(value) {
        root.settings=Object.assign({}, root.settings, {mountpoint:String(value||'/')})
        if(root.bar && root.bar.shell) root.bar.shell.updateEntryInline(root.moduleName,root.settings)
    }
    ...
    IpcHandler {
        ...
        function follow(value:string):void {root.setMountpoint(value)}
    }
    WidgetButton {
        ...
        tooltipText:'Disk Pulse · '+(root.stale?'Telemetry offline':(root.primary?Model.size(root.primary.free)+' free of '+Model.size(root.primary.total)+' on '+root.primary.mount:'no filesystem')+' · R '+Model.rate(root.rates.read)+' · W '+Model.rate(root.rates.write))+'\nLeft-click: dashboard · Right-click: readout'
        ...
        Row {
            ...
            Column {
                ...
                // Reserve the widest string the mode can produce, so a reading
                // that gains a digit does not re-lay out the whole bar section.
                width:Math.max(readoutText.implicitWidth,tagText.implicitWidth,widestReadout.implicitWidth,widestTag.implicitWidth)
                Text {id:readoutText;text:root.stale?'—':Model.readout(root.disk,root.mode,root.mountpoint);color:root.barForeground;font.family:Style.font.family;font.pixelSize:12;font.bold:true}
                Text {id:tagText;text:Model.modeTag(root.disk,root.mode);color:root.tint;font.pixelSize:7;font.letterSpacing:0.8}
                Text {id:widestReadout;visible:false;text:Model.widestReadout(root.mode);font.family:Style.font.family;font.pixelSize:12;font.bold:true}
                Text {id:widestTag;visible:false;text:Model.widestTag(root.mode);font.pixelSize:7;font.letterSpacing:0.8}
            }
        }
    }
    ...
    KeyboardPanel {
        ...
        Item {
            ...
            Keys.onPressed:function(event){
                ...
                if(root.chooseMode && event.key>=Qt.Key_1 && event.key<=Qt.Key_6){root.setMode(event.key-Qt.Key_1);event.accepted=true}
            }
            ...
            Column {
                ...
                Repeater {
                    ...
                    Action {
                        ...
                        text:(index+1)+'.  '+Model.modeName(index)+'   ·   '+Model.readout(root.disk,index,root.mountpoint)+(index===4?'  '+Model.modeTag(root.disk,4):'')
                        onClicked:root.setMode(index)
                    }
                }
                Action{text:'Open storage dashboard →';width:parent.width;onClicked:root.chooseMode=false}
            }
            Column {
                ...
                Row {
                    ...
                    Column {width:parent.width-220;spacing:3
                        Heading{text:'DISK PULSE';font.pixelSize:19;font.letterSpacing:3}
                        Label{text:'Your storage, in motion.';font.pixelSize:11}
                    }
                ...
                Row {spacing:8
                    Repeater {model:['Overview','Disk hogs','Storage lab','About']
