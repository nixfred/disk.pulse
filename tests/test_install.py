...
class InstallerTests(unittest.TestCase):
    ...
    def installer_fixture(self, config_text=None, missing=None):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in ('manifest.json', 'Panel.qml', 'Model.js', 'DiskChip.qml',
                         'HistoryGraph.qml', 'disk_pulse.py', 'README.md', 'disk-pulse.service'):
                if name != missing:
                    (source / name).write_text('new fixture: ' + name)
            config = home / '.config/omarchy/shell.json'
            dest = config.parent / 'plugins/nixfred.disk-pulse'
            ...
            data = {'bar': {'layout': {'left': [{'id': 'clock'}], 'center': [
                {'id': 'nixfred.disk-pulse', 'displayMode': 4, 'animated': False,
                 'mountpoint': '/home', 'custom': 'keep'}], 'right': []}}}
    ...
    def test_refuses_symlinked_config_directory(self):
        with self.installer_fixture() as (home, config, _, _, _, install):
            ...
            self.assertEqual(
                (real / 'plugins/nixfred.disk-pulse/Panel.qml').read_text(), 'old plugin')
...
class AboutIdentityTests(unittest.TestCase):
    ...
    def test_manifest_carries_version_repository_and_homepage(self):
        ...
        self.assertEqual(self.manifest['id'], 'nixfred.disk-pulse')
    ...
    def test_the_manifest_ships_with_the_plugin(self):
        ...
        self.assertEqual(install.PLUGIN_ID, self.manifest['id'])

    def test_service_runs_the_installed_daemon(self):
        unit = (ROOT / 'disk-pulse.service').read_text()
        self.assertIn('plugins/nixfred.disk-pulse/disk_pulse.py daemon', unit)
