...
test('the panel paints no colour of its own',()=>{
  // A single hex here is a surface that will stay wrong in every other theme.
...
test('the two canvases only hold colours as overridable defaults',()=>{
  for(const name of ['HistoryGraph.qml','DiskChip.qml']){
...
test('the shipped ramp is the one the other pulses ship',()=>{
  // Four chips on one bar must agree on what red, yellow and green mean.
  assert.deepEqual({...ctx.RAMP_FALLBACK},{low:'#850d29',mid:'#efcc45',high:'#43f2a1'});
});
